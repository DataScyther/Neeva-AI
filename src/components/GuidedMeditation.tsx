...
interface MeditationSession {
  ...
  type: 'breathing' | 'body-scan' | 'mindfulness' | 'loving-kindness' | 'focus';
...
export function GuidedMeditation() {
    ...
    if (isPlaying && selectedSession) {
      intervalRef.current = setInterval(() => {
        setCurrentTime(prev => {
          ...
          const instructionIndex = Math.floor((progressPercent / 100) * selectedSession.instructions.length);
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'body-scan': return Heart;
      case 'mindfulness': return Leaf;
      case 'loving-kindness': return Heart;
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
            ...
            <Card className="border-2 border-purple-200 bg-gradient-to-br from-purple-50 to-indigo-100">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className={`bg-gradient-to-r ${selectedSession.color} p-3 rounded-full`}>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="text-center space-y-4">
                  ...
                  <p className="text-muted-foreground">
                    {currentTime > 0 ? `${formatTime(currentTime)} elapsed` : 'Ready to begin'}
                  </p>
                </div>
                ...
                {selectedSession.backgroundSound && (
                  <div className="text-center">
                    <p className="text-sm text-muted-foreground">
                      🎵 Background: {selectedSession.backgroundSound.replace('-', ' ')}
                    </p>
                  </div>
                )}
              </CardContent>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-semibold">Choose Your Meditation</h2>
              ...
              <div key={session.id}>
                <Card 
                  className={`cursor-pointer transition-all hover:shadow-lg ${
                    isSelected ? 'border-2 border-purple-300 bg-purple-50' : ''
                  }`}
                ...
                >
                  <CardContent className="p-4">
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <div className={`bg-gradient-to-r ${session.color} p-2 rounded-full`}>
                      ...
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        ...
                        <span>{session.instructions.length} steps</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Lower cortisol levels and improved stress management
              </p>
            </div>
            <div className="text-center space-y-2">
      ...
      {!selectedSession && (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">Choose a meditation to begin</h3>
            <p className="text-muted-foreground">
              Select from our collection of guided meditations designed to support your mental wellbeing.
            </p>
          </CardContent>
        </Card>
      )}
