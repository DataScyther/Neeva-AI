...
const AuthComponent: React.FC<AuthComponentProps> = ({ onAuthSuccess }) => {
  ...
  const { register, handleSubmit, watch, reset, setValue, formState: { errors, isValid } } = useForm<AuthFormData>({
    resolver: zodResolver(authSchema),
    mode: 'onChange',
    defaultValues: { name: '', email: '', password: '' }
  });

  // Watch fields for real-time validation checks and suggestions
  ...
  useEffect(() => {
    if (email) {
      ...
      if (emailVal.suggestions && emailVal.suggestions.length > 0) {
        setEmailSuggestion(emailVal.suggestions[0]);
      } else {
        setEmailSuggestion('');
      }
    } else {
  ...
  // Form validity matching UX expectations (including password strength rules)
  const formValid = isValid && (authMode !== 'signup' || (passwordValidation?.strength !== 'weak' && passwordValidation?.isValid));

  const onSubmit = async (data: AuthFormData) => {
    const { name: formName, email: formEmail, password: formPassword } = data;
    ...
    if (authMode === 'signin') {
      ...
      try {
        ...
        await authService.signInWithEmail(formEmail, formPassword!);
      } catch (err: any) {
      ...
      const passwordVal = validatePassword(formPassword!, formEmail, formName);
      ...
      try {
        ...
        await authService.signUpWithEmail(formEmail, formPassword!, formName!.trim());
      } catch (err: any) {
      ...
      try {
        await authService.resetPassword(formEmail);
                      ...
                      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            disabled={isLoading}
                            className={`luxury-input h-12 ${
                              errors.email || (emailValidation && !emailValidation.isValid)
                            ...
                            <div className="flex items-start gap-1.5 text-xs text-blue-500 mt-1">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (suggestion) setValue('email', suggestion[1], { shouldValidate: true });
                                }}
